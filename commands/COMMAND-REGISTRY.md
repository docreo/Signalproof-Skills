...
| `build-git` | 0.2.3 | Execute the smallest governed Git-backed next action with current-Git, known-failure, PowerShell recurrence, protected-state, verification, and recovery controls |
...
See `commands/dsp.md` for the complete normalization and authority contract.
...
5. PowerShell-relevant build/continuation work must load the current Git-backed PowerShell failure registry, preserve same-failure attempt counts across chat boundaries, parser-preflight the exact final staged `.ps1`, and block unchanged known-bad retries.

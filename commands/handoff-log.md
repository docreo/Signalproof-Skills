...
The receiving workflow must reload current Git prevention data, reconcile any drift, parser-preflight the exact final staged `.ps1`, hash-bind the exact staged artifact when identity matters, preserve failed runners, and distinguish harness/parser/path/staging failures from product evidence.

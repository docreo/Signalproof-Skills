...
The problem is now runtime-confirmed across multiple Competition Radar surfaces, including a governed owner-entry form with several ComboBoxes. A bounded shared-template correction has now passed automated acceptance on the Build 2A selector surface, but owner visual recheck is still pending. The skill lifecycle therefore remains Candidate rather than Tested or Active.
...
## Semantic finding: unselected is not a governed value
...
- the `LIVE APPEND - BUILD 2B NOT AUTHORIZED` control remained unavailable as required and its text was readable.
...
Owner-supplied runtime screenshots from the design-git review:
...
This third runtime case strengthened the evidence that the defect belongs at the WPF visual-state/template layer rather than as a one-control cosmetic patch.

## Build 2A Design Correction 1 automated evidence

A bounded Build 2A Design Correction 1 then implemented the proposed governed-selector treatment on the Capability Assessment surface.

Automated acceptance proved:

- only `CapabilityAssessmentPage.xaml` and `CapabilityAssessmentPage.xaml.cs` changed;
- all 11 required governed selectors now begin with explicit no-selection cues;
- the cues are presentation-only `Select …` states rather than domain enum values;
- fake/default governed selections were removed;
- all 11 governed selectors initialize with `SelectedIndex = -1`;
- a shared custom dark ComboBox template/state matrix is present;
- popup ComboBox items use governed dark-state styling;
- product UI still does not invoke live `CommitAsync`;
- Build 2A write model remained unchanged;
- Build 2A one-shot coordinator remained unchanged;
- Build 2A disposable write/coordinator regression passed;
- Build 1 rating/promotion semantic regression passed;
- raw `radar.db`, `signals.db`, and `reviews.db` remained unchanged;
- live `capability-assessments.db` remained absent;
- live-write gate remained disabled;
- Build 2B live append remained unauthorized.

Accepted Design Correction 1 evidence identifiers:

- correction manifest SHA-256: `BC8228AB0521956F5AA28B4C8FF4DA4EA051977B0D32C935582CE69225519BB6`
- correction record SHA-256: `200806586406C418156D6B78A67D49B7BAF20F2FFE483F77BDBA29EA385151C8`
- staged/noncanonical correction ledger SHA-256: `2DC2B77BD65B2F9BE19D42984AE45731F456A280ABB789F6939B1E5CC10A5601`
- correction transcript SHA-256: `6085B928B0891EC96251FCB5F23E29E7B2C1AB11E45026E497152C0F8C90031B`

This is the first implementation evidence supporting the shared-state remedy. It is not yet sufficient for promotion because owner visual acceptance of the corrected collapsed/unselected/selected states is still pending.

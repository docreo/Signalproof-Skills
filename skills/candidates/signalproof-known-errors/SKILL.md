...
description: Public-safe catalog of repeatable Signalproof error fingerprints and known prevention or mitigation patterns. Use before consequential build, debug, test, verify, recovery, packaging, or release work to avoid repeating previously documented errors across tools, runtimes, platforms, and products.
---
...
`signalproof-known-errors` is the public Signalproof error catalog.

It contains only generalized, reusable error fingerprints and their supported prevention or mitigation patterns.

It intentionally excludes private Build Ledger chronology, private file paths, protected hashes, unreleased implementation details, customer data, secrets, private security evidence, and other private provenance.

Core rule:

> **Before repeating consequential work, check whether the failure pattern is already known.**

## Match rule

A known error applies only when the relevant conditions materially match. Compare tool/runtime/version/platform/action/symptom/boundary before applying a mitigation.

Do not diagnose from keywords alone.
